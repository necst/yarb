...
public class Minimization {
    ...
    public States orderStates(States states, Transitions tr){
    ...
    public States orderSerial(States states, Transitions tr){
    ...
    public States orderBridge(States states, Transitions tr) {
        States ordered = new States();
        System.out.println("States at start of orderBridge");
        states.showStatesLine();
        ...
        ordered.showStatesLine();
        //if the value of the state is == null and not equal to the number of paths, then add it to ordered list
        //otherwise add it to the list of lasts

        return ordered;
    }

    public States orderWeight(States states, Transitions tr) {
        ...
        for(State s: states.getAll()){
            ...
            for(Transition t : tr.getNextNoLoop(s).getAll()){
                if(!t.getInput().equals("empty")){
                    inI = inI + t.getInput().length();
                }
            }
            //System.out.println("Sum of sizes of entering transition labels: "+inI);
            for(Transition t : tr.getCurrNoLoop(s).getAll()){
                if(!t.getInput().equals("empty")){
                    outJ = outJ + t.getInput().length();
                }
            }
            //System.out.println("Sum of sizes of exiting transition labels: "+outJ);
            for(Transition t : tr.getLoop(s).getAll()){
                loop = loop + t.getInput().length();
            }
    ...
    private void followPath(State s, Transitions tr, States traversed){
        ...
        if(!s.getState().equals("end")){
            ...
            if(passStates.get(s)!=null){
                ...
                for(Transition t : tr.getCurr(s).getAll()){
