...
public class JSONReadFromFileArr {

    //private static ArrayList<Object> transitionList = new ArrayList<>()
	
	public JSONReadFromFileArr() { }

	//@SuppressWarnings("unchecked")
	public AutomatonArr getAutomaton(String file, int n) {
		System.out.println("Reading automaton "+n);
		AutomatonArr a = new AutomatonArr();
		...
		try {
			...
			for(int j = 0; j < transitions.size(); j++){
				...
				inp.add(input);
				a.addTransition(new TransitionArr(a.getAllStates().getById(curr), inp, a.getAllStates().getById(next)));
    ...
    public static TransitionsArr getIngoingTransitions(State s) {
		TransitionsArr tr = new TransitionsArr();
		//return only the transitions of the automaton n that has state s in them
        return tr;
    }

	public TransitionsArr getOutgoingTransitions(State s) {
		TransitionsArr tr = new TransitionsArr();
		//return only the transitions of the automaton n that has state s in them
        return tr;
	}
